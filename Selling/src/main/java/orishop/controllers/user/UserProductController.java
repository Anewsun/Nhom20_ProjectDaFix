...
//		}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String url = req.getRequestURI().toString();
		AccountModels user = (AccountModels) req.getSession().getAttribute("account");
		...
		if (url.contains("update")) {
			...
			if (user == null)
				resp.sendRedirect("http://localhost:8080/Selling/web/login");
			else
				postReview(req, resp);
		}
	}
	...
	public void getDetailProduct(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	    String pidParam = req.getParameter("pid");

	    if (pidParam == null || !pidParam.matches("^[0-9]+$")) {
	        // Nếu pid không hợp lệ, trả về mã lỗi hoặc chuyển hướng đến trang lỗi
	    ...
	    if (pro == null) {
	        // Nếu sản phẩm không tồn tại, trả về mã lỗi hoặc chuyển hướng đến trang lỗi
